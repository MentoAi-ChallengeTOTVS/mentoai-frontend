...
import { MOCK_REUNIOES, MOCK_ANALISES } from "@/mocks/reunioes";
 ...
 *
 * Client Component — precisa de `setInterval` pra simular as "atualizações
 * periódicas" sem um backend real fazendo polling. A cada tick: PENDENTE
 * vira PROCESSANDO, e PROCESSANDO avança progresso até "concluir" (vira
 * PROCESSADA e migra pra lista "Concluídas nesta sessão", cada item já
 * linkando pro Detalhe da Reunião — cobre a "navegação para análises
 * concluídas" do escopo). Estado é local à página (mesma limitação de
 * "sem estado global entre rotas" já documentada em Nova Reunião/Clientes/
 * Usuários) — reabrir a página reseta a simulação a partir do mock.
 */
...
function reuniao(id: number) {
  return MOCK_REUNIOES.find((r) => r.id === id)!;
}
    ...
    const intervalId = setInterval(() => {
      setEstado((prev) => {
        ...
        for (const item of prev.fila) {
          ...
          if (progresso >= 100) {
            novasConcluidas.push({ reuniaoId: item.reuniaoId, concluidoEm: agora });
          } else {
      ...
      <div className="flex w-full flex-col items-start gap-4">
        ...
        {fila.length === 0 ? (
          ...
          <div className="flex w-full flex-col items-start gap-3">
            {fila.map((item) => {
              const r = reuniao(item.reuniaoId);
              return (
                <div
                  key={item.reuniaoId}
              ...
              return (
                ...
                >
                  <div className="flex min-w-0 flex-1 flex-col items-start gap-0.5">
                    <p className="w-full truncate text-corpo text-navy">{r.cliente.nome}</p>
